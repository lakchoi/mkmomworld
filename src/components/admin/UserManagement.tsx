      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-stone-200 focus:outline-none focus:border-[#2dd4bf] text-sm"
          />
        </div>
        <select
          value={filterRole}
          onChange={(e) => { setFilterRole(e.target.value); }}
          className="px-4 py-2.5 rounded-xl border border-stone-200 focus:outline-none focus:border-[#2dd4bf] text-sm"
        >
          {roleOptions.map((opt) => (
        ...
        <button
          onClick={fetchUsers}
          disabled={loading}
          className="px-4 py-2.5 rounded-xl bg-rose-100 text-[#14b8a6] font-medium hover:bg-rose-200 transition-colors text-sm"
        >
          {loading ? "검색 중..." : "검색"}
        </button>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-rose-50">
              {users.map((user) => {
                ...
                return (
                  <tr key={user.id} className="hover:bg-[#ffffff]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="text-xs text-[#2dd4bf] hover:underline font-mono"
                      >
                        {user.referralCode}
                      </button>
                    </td>
