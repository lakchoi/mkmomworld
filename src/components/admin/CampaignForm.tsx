...
export function CampaignForm({ campaign, onClose, onSuccess }: CampaignFormProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-[#ccfbf1] overflow-hidden">
      <div className="flex items-center justify-between bg-[#2dd4bf] px-6 py-4">
        <h3 className="text-slate-800 font-bold text-lg">
          {campaign ? "캠페인 수정" : "새 캠페인 추가"}
        </h3>
        <button onClick={onClose} className="text-slate-800/80 hover:text-slate-800">
          <X size={20} />
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="캠페인 제목"
              className="w-full px-4 py-2.5 rounded-xl border border-stone-200 focus:outline-none focus:border-[#2dd4bf] focus:ring-2 focus:ring-[#2dd4bf]/20 text-sm"
            />
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-4 py-2.5 rounded-xl border border-stone-200 focus:outline-none focus:border-[#2dd4bf] focus:ring-2 focus:ring-[#2dd4bf]/20 text-sm resize-none"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              rows={8}
              className="w-full px-4 py-2.5 rounded-xl border border-stone-200 focus:outline-none focus:border-[#2dd4bf] focus:ring-2 focus:ring-[#2dd4bf]/20 text-sm resize-none font-mono"
            />
          </div>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input {...register("isActive")} type="checkbox" className="w-4 h-4 accent-rose-500" />
              <span className="text-sm font-medium text-stone-600">활성화</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input {...register("isDisplayed")} type="checkbox" className="w-4 h-4 accent-rose-500" />
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex-1 py-2.5 rounded-xl bg-[#2dd4bf] text-slate-800 font-bold hover:shadow-lg hover:shadow-[#2dd4bf]/20 transition-all disabled:opacity-70 flex items-center justify-center gap-2 text-sm"
          >
